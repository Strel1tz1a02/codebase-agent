...
def test_build_graph_runs_retrieve_tool_and_answer_flow_with_fake_dependencies():
    graph = build_graph()
    ...
    state["retriever"] = lambda question, repo_path, top_k: [
...
def test_build_graph_can_retrieve_again_before_answering():
    graph = build_graph()
    ...
    def fake_retriever(question, repo_path, top_k):
        retrieval_calls.append((question, repo_path, top_k))
        return [
            {
    ...
    state["retriever"] = fake_retriever
