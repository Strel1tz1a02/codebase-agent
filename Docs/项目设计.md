...
- `index_project()` 使用 `src/rag/code_loader.py`、`src/rag/chunker.py`、`src/rag/documents.py`、`src/rag/indexing.py`、`src/rag/vectorstores.py` 和 `src/rag/retrievers.py` 构建项目级 retriever。
- Runtime 保存 `project_id -> retriever` 或等价的项目级索引状态，Graph 执行时由 Runtime 注入 retriever。
