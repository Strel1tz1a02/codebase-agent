...
如果本机沿用历史环境名 `CodeBaseAgent`，执行命令时可替换为：
...
本次重构分为 9 个阶段。每个阶段都要有一个可验证的小目标，不把多个架构层同时打散。
...
8. **阶段 7：RAG 功能接入**
   小目标：把阶段 3 的新 RAG 模块接入 project index、Runtime、Graph、API 和 CLI 主链路。
...
**目标：** 建立 `Document -> Embedding -> VectorStore -> Retriever` 链路，并把 `retrieve_relevant_chunks()` 接到新链路。阶段 3 完成后，新版本 RAG 主路径不得再调用 `src.rag.embeddings.embed_chunks`、`src.rag.embeddings.embed_query_text`、`src.rag.index.build_index` 或 `src.rag.index.search_index`。

**阶段规则：**
- 旧 `src/rag/embeddings.py` 和 `src/rag/index.py` 只能作为迁移参考，不能作为阶段 3 后的新主路径依赖。
- `src/rag/retrieval.py::retrieve_relevant_chunks()` 如继续保留，只能作为兼容适配层，内部必须转发到 `documents/vectorstores/retrievers/indexing` 组成的新 RAG 链路。
- `src/tools/codebase.py::_retrieve_code()` 可以继续调用 `retrieve_relevant_chunks()`，但实际检索必须由新 RAG 模块完成。
- 本阶段不改 `src/graph`、`src/runtime`、`src/api`、`src/cli`。
...
- [ ] **Step 3.4：确认新 RAG 主路径不再引用旧链路**

Run:

```bash
rg "from src\.rag\.embeddings|from src\.rag\.index|embed_chunks|embed_query_text|build_index|search_index" src/rag src/tools tests
```

Expected:

```text
只允许旧兼容测试或待删除旧模块自身出现这些名称；`src/rag/retrieval.py`、`src/tools/codebase.py` 和新增的 `src/rag/documents.py`、`src/rag/vectorstores.py`、`src/rag/retrievers.py`、`src/rag/indexing.py` 不应依赖旧链路。
```
...
```bash
git add src/rag/documents.py src/rag/vectorstores.py src/rag/retrievers.py src/rag/indexing.py src/rag/retrieval.py tests/test_rag_documents.py tests/test_rag_vectorstores.py tests/test_rag_retrievers.py
...
阶段 6 暂不实现 `/tools` introspection 接口。工具注册表继续保留在工具层，后续如需要工具管理或调试，再单独设计对应 API。
...
**目标：** 阶段 6 已经完成 project/session/run/API/CLI 外壳，本阶段把阶段 3 的新 RAG 标准化模块接入真实主链路。完成后，`index` 不再只是修改 `index_status`，`ask` 也不再绕过项目级 RAG 索引。

**阶段规则：**
- 本阶段只接入已有新 RAG 模块，不重写 RAG。
- `src/rag/indexing.py` 只负责建索引，不接收用户问题，不执行召回。
- `src/rag/retrieval.py` 只负责从已有索引召回，不读取文件、不切 chunk、不创建 embedding、不构建 VectorStore。
- Runtime 是项目级 `RagIndex` 的持有者，Graph 只接收 `RagIndex` 并在召回节点调用 `retrieve_from_index()`。
- `POST /projects/{project_id}/index` 和 `python -m src.cli index` 必须走同一个 Runtime `index_project(project_id)`。
- 未索引项目执行 `ask` 必须抛出 `RagIndexNotReadyError`，不能静默返回空检索结果。
- 新版本不能回退到 `src/rag/embeddings.py`、`src/rag/index.py` 或旧的 `chunk -> embed_chunks -> build_index -> search_index` 主链路。
- 阶段 6 暂不保留 `/tools` 的规则不变，本阶段不新增 `/tools`。
...
```text
CLI/API index
  -> RuntimeService.index_project(project_id)
  -> rag.indexing.build_project_index(project_id, repo_path, config)
  -> Runtime 保存 project_id -> RagIndex
```
...
```text
CLI/API ask
  -> RuntimeService.ask(project_id, session_id, question)
  -> Runtime 读取 project_id 对应的 RagIndex
  -> Graph retrieve_context
  -> rag.retrieval.retrieve_from_index(index, question, top_k)
```

**Files:**
- Modify: `src/runtime/projects.py`
- Modify: `src/runtime/store.py`
- Modify: `src/runtime/runs.py`
- Modify: `src/rag/indexing.py`
- Modify: `src/rag/retrieval.py`
- Modify: `src/graph/nodes.py`
- Modify: `src/api/routes_projects.py`
- Modify: `src/cli/main.py`
- Test: `tests/test_rag_indexing.py`
...
- [ ] **Step 7.1：定义 RagIndex 和 RagHit**

在 `src/rag/schemas.py` 中定义 RAG 层自己的领域对象：
...
```bash
E:\anaconda\envs\CodeBaseAgent\python.exe -m pytest tests/test_rag_indexing.py tests/test_rag_retrieval.py -v
```

Expected:

```text
FAIL，如果测试还未覆盖 RagIndex/RagHit；下一步补测试。
```

- [ ] **Step 7.2：写 build_project_index 测试**

在 `tests/test_rag_indexing.py` 中新增测试，确认建索引只产出 `RagIndex`：
...
```bash
E:\anaconda\envs\CodeBaseAgent\python.exe -m pytest tests/test_rag_indexing.py -v
```

Expected:

```text
FAIL，因为 build_project_index 尚未实现。
```

- [ ] **Step 7.3：实现 build_project_index**

在 `src/rag/indexing.py` 中新增 `build_project_index()`。这个函数可以调用 loader、chunker、documents、embedding、vector store 和 `index_documents()`，但不能接收 question，也不能做召回：

```python
def build_project_index(project_id: str, repo_path: str, config: AppConfig | None = None) -> RagIndex:
...
- [ ] **Step 7.4：写 retrieve_from_index 测试**

在 `tests/test_rag_retrieval.py` 中新增测试，确认召回只消费已有 `RagIndex`：
...
def test_retrieve_from_index_returns_hits_without_rebuilding(tmp_path):
    repo_file = tmp_path / "app.py"
    repo_file.write_text("def entrypoint():\n    return 'ok'\n", encoding="utf-8")
    index = build_project_index("demo", str(tmp_path))
...
```bash
E:\anaconda\envs\CodeBaseAgent\python.exe -m pytest tests/test_rag_retrieval.py -v
```

Expected:

```text
FAIL，因为 retrieve_from_index 尚未实现。
```

- [ ] **Step 7.5：实现 retrieve_from_index**

在 `src/rag/retrieval.py` 中新增 `retrieve_from_index()`。这个函数只能从 `RagIndex.vector_store` 构造 retriever 并查询：

```python
def retrieve_from_index(index: RagIndex, question: str, top_k: int = 5) -> list[RagHit]:
    ...
    return [
        ...
        for document in documents[: max(1, int(top_k))]
    ]
```

`retrieve_from_index()` 不允许调用 `load_code_files()`、`chunk_code_files()`、`build_embeddings()`、`build_local_vector_store()` 或 `index_documents()`。
...
- [ ] **Step 7.6：写 Runtime RAG 索引测试**

在 `tests/test_runtime_projects.py` 中新增测试，确认 `index_project()` 会构建项目级 `RagIndex`，并更新 index 状态：
...
```bash
E:\anaconda\envs\CodeBaseAgent\python.exe -m pytest tests/test_runtime_projects.py -v
```

Expected:

```text
FAIL，因为 RuntimeService 尚未实现 index_project 和 get_project_index。
```

- [ ] **Step 7.7：实现 Runtime 项目级 RAG 索引**

在 Runtime 层新增 `index_project(project_id)`：

```python
def index_project(self, project_id: str) -> Project:
    project = self.get_project(project_id)
    rag_index = build_project_index(project.project_id, project.repo_path)
    self.store.project_indexes[project.project_id] = rag_index
    project.index_status = "indexed"
    return project
```

Runtime 只能调用 `build_project_index()`，不能在 Runtime 里拼 loader、chunker、Document、VectorStore。
...
- [ ] **Step 7.8：写未索引 ask 失败测试**

在 `tests/test_runtime_runs.py` 中新增测试：
...
```bash
E:\anaconda\envs\CodeBaseAgent\python.exe -m pytest tests/test_runtime_runs.py -v
```

Expected:

```text
FAIL，因为 ask 尚未检查项目索引状态。
```

- [ ] **Step 7.9：让 Runtime 在 graph state 中注入 RagIndex**

修改 `_run_graph()`：

```python
rag_index = self.get_project_index(project.project_id)
...
```bash
E:\anaconda\envs\CodeBaseAgent\python.exe -m pytest tests/test_runtime_runs.py tests/test_graph_nodes.py -v
```
...
- [ ] **Step 7.10：让 Graph retrieve_context 调用 retrieve_from_index**

修改 `src/graph/nodes.py::retrieve_context()`：

```python
rag_index = state.get("rag_index")
if rag_index is not None:
    hits = retrieve_from_index(
        rag_index,
        _latest_user_question(state),
        int(state.get("retrieval_top_k", 5)),
    )
    messages = [ToolMessage(content=format_hits(hits), name="retrieve_context", tool_call_id="retrieve_context:1")]
else:
    messages = [ToolMessage(content="无", name="retrieve_context", tool_call_id="retrieve_context:1")]
return {"messages": messages}
```

Graph 只能调用 `retrieve_from_index()`，不能调用 `build_project_index()`。

Run:

```bash
E:\anaconda\envs\CodeBaseAgent\python.exe -m pytest tests/test_graph_nodes.py -v
```
...
- [ ] **Step 7.11：让 API index 调用 Runtime**

修改 `src/api/routes_projects.py`，`POST /projects/{project_id}/index` 不再直接写 `project.index_status = "indexed"`，而是：

```python
project = app.state.runtime.index_project(project_id)
```

新增或更新 API 测试，断言 index 接口能把项目变为 `indexed`，并且未知 project 仍返回 404。

Run:

```bash
E:\anaconda\envs\CodeBaseAgent\python.exe -m pytest tests/test_api_projects.py -v
```
...
- [ ] **Step 7.12：让 CLI index 调用 Runtime**

修改 `src/cli/main.py`，`_run_index()` 创建 project 后调用：

```python
project = runtime.index_project(project.project_id)
```

同时确保 `_run_ask()` 在未索引时不会吞掉 `RagIndexNotReadyError`；如果 CLI 需要捕获错误，必须输出明确结构化错误。

Run:

```bash
E:\anaconda\envs\CodeBaseAgent\python.exe -m pytest tests/test_cli_main.py -v
```
...
- [ ] **Step 7.13：确认 index 和召回没有混在一起**

Run:

```bash
rg "load_code_files|chunk_code_files|build_embeddings|build_local_vector_store|index_documents|build_project_index" src/rag/retrieval.py src/graph src/runtime src/api src/cli
```

Expected:

```text
`src/rag/retrieval.py` 和 `src/graph` 不应出现建索引相关函数；Runtime 只允许出现 build_project_index，不允许直接出现 loader/chunker/vector store 拼装。
```

- [ ] **Step 7.14：确认新路径没有回退到旧 RAG 链路**

Run:

```bash
rg "from src\.rag\.embeddings|from src\.rag\.index|embed_chunks|embed_query_text|build_index|search_index" src/runtime src/graph src/api src/cli src/tools tests/test_runtime_projects.py tests/test_runtime_runs.py tests/test_api_projects.py tests/test_cli_main.py
```

Expected:

```text
无输出；如果有输出，只允许是旧模块自身或明确标记为迁移参考的旧测试，不允许出现在 Runtime、Graph、API、CLI 主链路中。
```
...
```bash
git add src/rag src/runtime src/graph src/api/routes_projects.py src/cli/main.py tests/test_rag_indexing.py tests/test_rag_retrieval.py tests/test_runtime_projects.py tests/test_runtime_runs.py tests/test_graph_nodes.py tests/test_api_projects.py tests/test_cli_main.py
git commit -m "feat: connect rag indexing to runtime workflow"
```
