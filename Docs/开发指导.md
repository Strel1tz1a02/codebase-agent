...
- Runtime 是 RAG 索引和 retriever 的持有者，Graph 只接收并调用 retriever。
...
- [ ] **Step 7.1：写 Runtime RAG 索引测试**

在 `tests/test_runtime_projects.py` 中新增测试，确认 `index_project()` 会构建项目级 retriever，并更新 index 状态：

```python
def test_runtime_index_project_builds_retriever(tmp_path):
    ...
    assert runtime.get_project_retriever(project.project_id) is not None
```
...
```text
FAIL，因为 RuntimeService 尚未实现 index_project 和 get_project_retriever。
```

- [ ] **Step 7.2：实现 Runtime 项目级 RAG 索引**
...
def index_project(self, project_id: str) -> Project:
    ...
    files = load_code_files(project.repo_path)
    ...
    self.store.project_retrievers[project.project_id] = build_retriever(vector_store, config.retrieval.top_k)
    project.index_status = "indexed"
    return project
```

如果现有函数签名与上面示例不完全一致，以现有 `src/rag/*` 模块为准，但依赖方向必须保持为 Runtime 调用 RAG，Graph 不构建索引。
...
- [ ] **Step 7.4：让 Runtime 在 graph state 中注入 retriever**

修改 `_run_graph()`：

```python
retriever = self.get_project_retriever(project.project_id)
...
- [ ] **Step 7.5：让 API index 调用 Runtime**
...
- [ ] **Step 7.6：让 CLI index 调用 Runtime**
...
```bash
git add src/runtime src/graph src/api/routes_projects.py src/cli/main.py tests/test_runtime_projects.py tests/test_runtime_runs.py tests/test_graph_nodes.py tests/test_api_projects.py tests/test_cli_main.py
